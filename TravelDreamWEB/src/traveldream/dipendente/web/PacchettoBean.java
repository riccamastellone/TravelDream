...
public class PacchettoBean implements Serializable {
	...
	public String aggiungiPacchetto() throws ParseException {
		...
		// PRIMO STEP: aggiungo le info generali del pacchetto a db e ricavo l
		// id giudto del pacchetto
		this.pacchetto = pkgMng.salvaInfoGenerali(pacchetto);
		...
		// SESTO STEP: verifico se un hotel e nuovo oppure esistente(id !=0), se
		// nuovo lo inserisco a db
	...
	public void mostraInfo(AjaxBehaviorEvent actionEvent, PacchettoDTO pacchetto) {
		 ...
		 * System.out.println("RITORNO"); for (VoloDTO volo :
		 * this.pacchettoDaVisualizzareDto.getVoliRitorno()) {
		 * System.out.println(volo.getNomeCompagnia()); }
		 */
	}
	...
	public List<PacchettoDTO> getPacchettiVendibili() {
		if (this.pacchettiVendibili.isEmpty()) {
			this.pacchettiVendibili = pkgMng.getPacchettiVendibili();
		}
		return this.pacchettiVendibili;
	}

	public void setPacchettiVendibili(List<PacchettoDTO> pacchettiVendibili) {
		this.pacchettiVendibili = pacchettiVendibili;
	}
