...
@NamedQueries({
	...
	@NamedQuery(name="Pacchetto.Ricerca", query="SELECT p FROM Pacchetto p Where p.eliminato !=1 AND (p.localita Like :localita OR p.nome Like :localita) AND p.hotel != NULL"),
	
	@NamedQuery(name="Pacchetto.getPacchettiByHotel", query="SELECT p FROM Pacchetto p WHERE p.hotel = :hotel")
